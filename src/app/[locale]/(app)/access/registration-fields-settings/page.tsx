...
// src/app/[locale]/(app)/access/registration-fields-settings/page.tsx
"use client";

import * as React from "react";
import { useForm, useFieldArray, UseFormReturn } from "react-hook-form";
import { useTranslations } from 'next-intl';
import { useNxRegistrationFields } from '@/hooks/use-nx-registration-fields';
...
import { Loader2, Settings, PlusCircle, Trash2, GripVertical, ShieldCheck, Eye, Asterisk, MoreVertical, Edit, RefreshCw } from "lucide-react";
...
const FieldTypeEnumOptions = ["text", "number", "email", "date", "dropdown", "checkbox", "switch", "file", "cpf", "cnpj", "address_group", "url", "textarea", "social"];

const FieldEditor: React.FC<{ form: UseFormReturn<any>, index: number, isSystemDefault: boolean, disabled: boolean }> = ({ form, index, isSystemDefault, disabled }) => {
    const t = useTranslations('userRegistrationFields.fieldEditor');
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 pt-4">
            ...
            <FormField control={form.control} name={`fields.${index}.fieldKey`} render={({ field }) => ( <FormItem><FormLabel>{t('fieldKeyLabel')}</FormLabel><FormControl><Input {...field} disabled={isSystemDefault || disabled} /></FormControl><FormDescription>{t('fieldKeyDescription')}</FormDescription><FormMessage /></FormItem> )} />
            <FormField control={form.control} name={`fields.${index}.fieldType`} render={({ field }) => ( <FormItem><FormLabel>{t('fieldTypeLabel')}</FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isSystemDefault || disabled}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>{FieldTypeEnumOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem> )} />
            <FormField control={form.control} name={`fields.${index}.description`} render={({ field }) => ( <FormItem><FormLabel>{t('descriptionLabel')}</FormLabel><FormControl><Textarea {...field} value={field.value ?? ""} placeholder={t('descriptionPlaceholder')} rows={2} disabled={disabled}/></FormControl><FormMessage /></FormItem> )} />
             <div className="md:col-span-2 flex flex-wrap items-center gap-4 mt-2">
                ...
                {(field.fieldType === 'cpf') && (<FormField control={form.control} name={`fields.${index}.validationConfig.active`} render={({ field: formField }) => ( <FormItem className="flex items-center space-x-2 p-3 border rounded-md bg-sky-50 dark:bg-sky-900/30"><FormControl><Switch checked={formField.value} onCheckedChange={formField.onChange} disabled={disabled}/></FormControl><FormLabel className="text-sm mb-0">{t('validationLabel')}</FormLabel></FormItem> )}/>)}
                {field.fieldKey === 'profilePictureUrl' && field.fieldType === 'file' && ( <FormField control={form.control} name={`fields.${index}.useOAuthPhoto`} render={({ field: formField }) => ( <FormItem className="flex items-center space-x-2 p-3 border rounded-md"><FormControl><Switch checked={formField.value} onCheckedChange={formField.onChange} disabled={disabled}/></FormControl><FormLabel className="text-sm mb-0">{t('useOAuthPhotoLabel')}</FormLabel></FormItem> )} />)}
            </div>
        </div>
    );
};
...
export default function UserRegistrationFieldsSettingsPage() {
  ...
  if (contextType === 'master' && !canEdit) {
      return (
          <Card>
              <CardHeader>
                  <BackButton />
                  <CardTitle><Settings className="mr-2 h-6 w-6 inline-block" />{t('pageTitle')}</CardTitle>
            ...
            <CardContent>
              <SortableList
                    ...
                    renderItem={(field, { attributes, listeners, isDragging }) => {
                        ...
                        const isSystemDefault = field.isSystemDefaultField;
                                    ...
                                    <div className="flex items-center p-2 rounded-t-md data-[state=open]:bg-muted/30">
                                        ...
                                        <AccordionTrigger className="flex-1 p-2 hover:no-underline" onClick={() => setOpenAccordion(prev => prev.includes(field.id) ? prev.filter(id => id !== field.id) : [...prev, field.id])}>
                                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                                ...
                                                {isSystemDefault && (<TooltipProvider><Tooltip><TooltipTrigger asChild><span onClick={(e) => e.stopPropagation()}><ShieldCheck className="h-4 w-4 text-blue-600 shrink-0" /></span></TooltipTrigger><TooltipContent><p>{t('fieldItem.tooltipSystemField')}</p></TooltipContent></Tooltip></TooltipProvider>)}
                                            </div>
                                        </AccordionTrigger>
                                        <div className="flex items-center gap-0.5 shrink-0">
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent>
                                                    ...
                                                    <DropdownMenuItem className="text-destructive" disabled={isSystemDefault} onClick={() => onDeleteField(index)}><Trash2 className="mr-2 h-4 w-4"/>{t('fieldItem.menuDelete')}</DropdownMenuItem>
                                                </DropdownMenuContent>
                                            </DropdownMenu>
                                        </div>
                                    </div>
                                    <AccordionContent className="px-4 pb-4 border-t bg-background rounded-b-md">
                                        <FieldEditor form={form} index={index} isSystemDefault={!!isSystemDefault} disabled={!!isPageEffectivelyDisabled} />
                                    </AccordionContent>
                                </AccordionItem>
                            </Accordion>
                        )
