...
import { PilcrowSquare, Info, Loader2, Save, Wand2, GripVertical, PlusCircle, Trash2, ChevronDown, MoreVertical, FileJson, AlertTriangle } from 'lucide-react';
...
export default function TiptapEditorPage() {
  ...
  const tToolbar = useTranslations('tiptapEditor.toolbar');
  const { hasPermission } = useUserPermissions();

  const canConfigure = hasPermission('master.modules.edit');
  const canManagePresets = hasPermission('master.modules.edit');

  const initialContent = useMemo(() => {
    const strong = (chunks: React.ReactNode) => <strong>{chunks}</strong>;
    const p1 = t.rich('initialContent.p1', { strong });
    return `<h2>${t('initialContent.h2')}</h2><p>${p1}</p><ul><li><p>${t('initialContent.li1')}</p></li><li><p>${t('initialContent.li2')}</p></li><li><p>${t('initialContent.li3')}</p></li></ul><p>${t('initialContent.p2')}</p>`;
  }, [t]);
  ...
  const toolbarConfigDocRef = useMemo(() => doc(refs.master.tiptapEditorModuleCollection(), 'default_toolbar_config'), []);
  const presetsCollectionRef = useMemo(() => refs.master.tiptapEditorPresets(), []);

  const fetchConfig = useCallback(async () => {
    setIsLoading(true);
    try {
      const docSnap = await getDoc(toolbarConfigDocRef);
      const baseOptions = getTranslatedOptions(defaultTiptapOptions);
      if (docSnap.exists()) {
      ...
      setOptions(getTranslatedOptions(defaultTiptapOptions));
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        const fetchedPresets = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Preset));
  ...
  const handleMove = (currentIndex: number, direction: 'up' | 'down') => {
    ...
    setDisplayOptions(prev => arrayMove(prev, currentIndex, newIndex));
    setHasUnsavedChanges(true);
  };

  const handleAddSeparator = () => {
    if (!canConfigure) return;
    const newSeparator: EditorOption = { key: `separator_${Date.now()}`, label: t('separatorLabel'), icon: 'Minus', active: true };
  ...
  const executeSavePreset = async (presetName: string) => {
    ...
    setIsSaving(true);
    try {
        const presetDocRef = doc(presetsCollectionRef, presetName);
        const optionsToSave = displayOptions.map(({ label, ...rest }) => rest);
        await setDoc(presetDocRef, { name: presetName, content: content, settings: { toolbar: optionsToSave }, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true });
                                              ...
                                              <div className={cn("flex items-center gap-2 p-2 border rounded-md", isSeparator ? "bg-muted/50" : "bg-background")}>
                                                  ...
                                                  {isSeparator ? (
                                                      ...
                                                      <>
                                                          ...
                                                          <Switch id={opt.key} checked={opt.active} onCheckedChange={(checked) => handleOptionToggle(opt.key, checked)} disabled={!canConfigure}/>
                                                      </>
                                                  )}
                                                  <div className="flex items-center gap-1 ml-auto">
                                                      ...
                                                      />
                                                      {isSeparator && (
                                                          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleRemoveOption(opt.key)} disabled={!canConfigure}><Trash2 className="h-4 w-4"/></Button>
                                                      )}
                                                  </div>
                                              </div>
                                              );
                              ...
                              <div className="flex flex-col sm:flex-row justify-between sm:items-center w-full gap-2 px-4 py-3">
                                  ...
                                  <Dialog open={isSavePresetDialogOpen} onOpenChange={setIsSavePresetDialogOpen}>
                                      <DialogTrigger asChild>
                                          <Button variant="outline" size="sm" className="w-full sm:w-auto shrink-0" disabled={!canManagePresets}>
                                      ...
                                      <DialogContent className="sm:max-w-md p-0">
                                          <form onSubmit={(e) => { e.preventDefault(); handleSavePreset(newPresetName); }}>
                                              ...
                                              <div className="px-6 space-y-2">
                                                  <Label htmlFor="preset-name">{t('dialogs.savePreset.label')}</Label>
                                              ...
                                              <DialogFooter className="p-6 pt-4 border-t">
                                                  ...
                                                  <Button type="submit" disabled={isSaving || !canManagePresets}>
