# Documentação da Página: Módulo de Editor Lexical

Este documento detalha o funcionamento e a lógica da página de "Módulo de Editor (Lexical)", localizada em `src/modules/lexical-editor/page.tsx`.

---

## 1. Visão Geral e Propósito

A página do **Módulo de Editor Lexical** serve a dois propósitos principais:

1.  **Configuração da Barra de Ferramentas:** Permite que o administrador do sistema personalize a barra de ferramentas do editor, ativando ou desativando cada funcionalidade (botão) disponível, como Negrito, Itálico, Listas, Alinhamento, etc. Também permite a reordenação das ferramentas com drag-and-drop.
2.  **Gerenciamento de Presets:** Oferece uma interface para salvar a configuração atual da barra de ferramentas como um "preset" reutilizável, carregar presets existentes ou excluí-los.
3.  **Demonstração e Teste:** Fornece uma instância funcional do editor de texto para que as configurações possam ser testadas em tempo real.

## 2. Lógica de Funcionamento

### a. Gerenciamento de Configuração
-   **Fonte de Dados:** As configurações da barra de ferramentas são salvas em um único documento no Firestore, localizado em `Global/master/config/modules/lexical-editor/default_toolbar_config`.
-   **Carregamento:** Ao carregar, a página busca este documento. Se ele existir, a interface é populada com as configurações salvas. Se não, utiliza as configurações padrão.
...
-   **Fonte de Dados:** Os presets são salvos em `Global/master/config/modules/lexical-editor/presets`.
-   **Ações:** Através de um menu de contexto, o usuário pode carregar a configuração de um preset para o formulário, sobrescrever um preset existente com a configuração atual, ou excluir um preset.
-   **Confirmação:** Ações de sobrescrever e excluir exigem confirmação do usuário através de uma caixa de diálogo para evitar ações acidentais.

### c. Editor de Preview Dinâmico
-   O componente principal do editor Lexical é renderizado abaixo da seção de configuração.
-   A barra de ferramentas (`ToolbarPlugin.tsx`) recebe a lista de opções ativas como uma propriedade (`activeOptions`) e renderiza dinamicamente apenas os botões e controles que correspondem às opções ativas, fornecendo um feedback visual imediato.
...
import { AudioLines, Loader2, Save, Wand2, GripVertical, PlusCircle, Trash2, ChevronDown, MoreVertical } from 'lucide-react';
...
export default function LexicalEditorModulePage() {
  ...
  const presetsDocRef = useMemo(() => {
    return doc(refs.master.lexicalEditorModuleCollection(), 'presets');
  }, []);
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const optionKeys = new Set(defaultLexicalOptions.map(o => o.key));
        
        const mergedOptions = defaultLexicalOptions.map(defaultOpt => {
        ...
        savedOptions.forEach(savedOpt => {
            if (!optionKeys.has(savedOpt.key)) {
                mergedOptions.push(savedOpt);
            }
        });

        setOptions(mergedOptions);
      } else {
        setOptions(defaultLexicalOptions);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(presetsDocRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        const fetchedPresets = Object.keys(presetsMap).map(key => ({
          id: key,
          name: key, 
          settings: presetsMap[key]
        }));
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      toast({ title: "Configuração Padrão Salva!", description: "A barra de ferramentas do editor foi atualizada." });
      setHasUnsavedChanges(false);
  ...
  const handlePresetSelect = (presetName: string) => {
    ...
    if (selectedPreset?.settings?.toolbar) {
        ...
        toast({
            ...
            description: `A configuração "${presetName}" foi aplicada. Clique em 'Salvar Padrão' para confirmar.`
        });
    }
  };
            ...
            <CardHeader className="relative">
                ...
                <div className="pt-2">
                    <CardTitle className="section-title !border-none !pb-0">
                        ...
                        Módulo de Editor (Lexical)
                    </CardTitle>
                    <CardDescription>
                      Configure a barra de ferramentas e teste o editor de texto rico baseado no framework Lexical.
                    </CardDescription>
                </div>
            </CardHeader>
            <CardContent>
              ...
              <LexicalComposer initialConfig={initialConfig}>
                    <div className="border rounded-md min-h-[300px] flex flex-col">
                        <ToolbarPlugin activeOptions={activeOptions} />
                        ...
                        <RichTextPlugin
                            ...
                            placeholder={<div className="absolute top-4 left-4 text-muted-foreground pointer-events-none">Digite aqui para testar...</div>}
